...
module DSP.Artery.Filter.Biquad (biquadFilter, lowpass, highpass, bandpass, notch, allpass, peaking) where
...
biquadHelper :: (Floating a, Given SampleRate) => (a -> a -> BiquadParam a) -> a -> a -> BiquadParam a
biquadHelper f freq q = let omega = 2 * pi * freq / getSampleRate given in f omega (sin omega / q / 2)
{-# INLINE biquadHelper #-} 
